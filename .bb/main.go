	...
	w.Job(bb.NewJob().
		...
		OnSuccess(func(event *bb.JobStatusChangedEvent) {
			...
			goDockerConfig := bb.NewDocker().
				Image(fmt.Sprintf("greg-tower:5000/go-builder:%s", "latest")). //*jGraph.Job.Fingerprint)).
